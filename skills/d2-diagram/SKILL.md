...
When a diagram teaches a new concept, establish the learner's problem and
define the central idea in plain language before asking them to interpret
the picture. Explain what its entities and connections mean. A diagram with
unfamiliar labels is not a substitute for a definition or a reason to care.
For a narrated lesson, make that meaning available both visually and in the
spoken explanation.

Use the smallest example that exposes the mechanism before adding the source
material's implementation detail. Keep semantic roles recognizable as the
example grows, explicitly map changed names, and connect diagram elements to
the corresponding code or worked state. Review whether the learner can
explain why the mechanism helps without tracing a full listing. Render and
inspect the actual source; show that rendered asset in the lesson rather
than an independently redrawn approximation.

This guidance is for concept teaching. It does not require a toy example,
lesson sequence, or quiz for an ordinary architecture or relationship map.
